...
function callClaude(userMsg){
  ...
  // ── v4.3: 금융 상식 (홈 💡 버튼 + 직접 질문) — 매번 다른 꿀팁 1개 ──
  if(msg.includes('금융상식') || (msg.includes('금융') && msg.includes('상식')) || msg.includes('돈꿀팁') || (msg.includes('금융') && msg.includes('꿀팁'))){
    const FIN_TIPS = [
      '💡 <b>주휴수당</b> — 주 15시간 이상 일하면 하루치 급여를 더 받을 수 있어. 시급 10,320원 기준으로 주 5일 8시간이면 매주 82,560원이 추가되는 거야!',
      '💡 <b>연장근무 1.5배</b> — 하루 8시간을 넘긴 근무는 시급의 1.5배로 계산돼야 해. 5인 이상 사업장이라면 꼭 챙기자!',
      '💡 <b>야간수당</b> — 밤 10시~새벽 6시 근무는 시급의 0.5배가 추가돼. 야간 알바라면 급여명세서에서 꼭 확인!',
      '💡 <b>3.3% 원천징수</b> — 프리랜서·일용직이 떼인 3.3% 세금은 5월 종합소득세 신고하면 대부분 환급받을 수 있어!',
      '💡 <b>청년도약계좌</b> — 만 19~34세면 월 70만원까지 납입 가능, 정부 기여금 더해서 5년 뒤 최대 5,000만원 목돈 만들기!',
      '💡 <b>비상금 규칙</b> — 월 지출의 3배를 비상금으로 먼저 모으는 게 재테크의 시작이야. 머니냥의 생존관리로 지출부터 파악해보자!',
      '💡 <b>50:30:20 법칙</b> — 수입의 50%는 필수지출, 30%는 자유지출, 20%는 저축으로 나누면 돈 관리가 쉬워져!',
      '💡 <b>퇴직금</b> — 1년 이상 일했다면 알바도 퇴직금을 받을 수 있어 (주 15시간 이상). 30일치 평균임금 × 근속연수야!',
      '💡 <b>실업급여</b> — 고용보험 180일 이상 가입 후 비자발적 퇴사라면 실업급여 대상이야. 알바도 고용보험 가입돼 있다면 가능!',
      '💡 <b>소액 자동저축</b> — 월급날 자동이체로 먼저 저축하고 남은 돈으로 생활하는 "선저축 후지출"이 가장 확실한 방법이야!'
    ];
    const tip = FIN_TIPS[Math.floor(Math.random()*FIN_TIPS.length)];
    const r = `${greeting}오늘의 금융 상식이야! 🐱\n\n${tip}\n\n💬 _(또 눌러봐, 다른 꿀팁도 알려줄게!)_`;
    chatHistory.push({ role: 'bot', text: r }); lsSave(); return r;
  }
