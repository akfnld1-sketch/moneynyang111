...
function renderIncomeCalc(){
  ...
  // v2.0: 연봉제 단독 선택 → 연봉제 전용 페이지 (N잡 입력 UI 불필요)
  const isSalaryOnly = selectedJobs.length===1 && selectedJobs[0]==='salary';
  if(isSalaryOnly){
    page.innerHTML = `
    <div style="margin-bottom:16px;">
      <h2 style="font-size:26px;font-weight:700;">💼 ${curY}년 ${curM+1}월 수입관리${typeof helpBtn==='function'?helpBtn('sal'):''}</h2>
      <div style="font-size:15px;color:var(--text3);margin-top:2px;">연봉제 급여 — 4대보험·세금 자동계산</div>
    </div>
    ${(typeof renderAnnualSalaryCardHTML==='function') ? renderAnnualSalaryCardHTML() : ''}
    <div style="background:rgba(255,209,102,.08);border:1px solid rgba(255,209,102,.25);border-radius:10px;padding:12px 16px;font-size:15px;color:var(--text3);line-height:1.8;">
      ※ 본 계산은 <b style="color:var(--yellow)">참고용</b>입니다. 간이세액표 근사치로 실제 공제액과 차이가 있을 수 있어요.<br>
      정확한 금액은 회사 급여명세서 또는 국세청 간이세액표를 확인하세요.
    </div>`;
    return;
  }
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
    ...
    ${renderWageSettings(selectedJobs.filter(j=>j!=='employee'&&j!=='salary'), wages)}
  </div>
